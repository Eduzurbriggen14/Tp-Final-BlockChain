...
// Transferencia de NFT de un usuario a otro.
    event Trade(address indexed buyer, uint256 indexed seller, uint256 indexed tokenId, uint256 value);
...
// Verificacón del valor total después de la compra (no debe exeder el valor máximo permitido "maxValueToRaise"). Incluir un mensaje de falla.
        require(totalValue + value <= maxValueToRaise, "El total de ntf supera el valor maximo");
        ...
        for (uint256 i=0; i<amount; i++) { // Bucle desde 1 hasta amount (inclusive) para mintear la cantidad especificada de NFTs.
            ...
            _safeMint(_mgsender(),tokenIdTracker.current()); // Minteo de NFT y asignación al msg.sender.
            emit Buy(_mgsender(),tokenIdTracker.current(), value); // Evento Buy con el comprador, el tokenId y el valor del NFT.
...
// Parámetros: Lista de IDs de tokens de reclamo (utilizar calldata).
    function claim(uint256[]calldata listTokenId) external nonReentrant {
        ...
        require(listTokenId.length > 0 && listTokenId.length <= maxBatchCount, "Cantidad a reclamar invalida"); // Verificacón de la cantidad de tokens a reclamar (mayor que 0 y menor o igual a maxBatchCount). Incluir un mensaje de falla.
        uint256 claimValue = 0; // Inicializacion de claimValue a 0.
        TokenSale storage tokenSale; // Variable tokenSale.
        for (uint256 i=0; i<lestTokenId.length; i++) { // Bucle para iterar a través de cada token ID en listTokenId.
...
// Verificamos que el llamador de la función (_msgSender()) sea el propietario del token. Si no es así, la transacción falla con el mensaje "Only owner can Claim".
            ...
            removeFromArray(listTokenOnSale, listTokenId[i]); // Remover el token de la lista de tokens en venta.           
        ...
        totalValues -= claimValue                                ; // Reducir el totalValue acumulado.
...
// Funcion de compra de NFT que esta en venta.
    function trade(unint256 tokenId) external nonReentrant { // Parámetro: ID del token.
...
// Verificamos que el comprador (el que llama a la función) no sea el propietario actual del NFT. Si lo es, la transacción falla con el mensaje "Buyer is the Seller".
        require(_msgSender() != ownerOf(tokenId), "El comprador ya es propietario del token"); // Verificación de propietario actual del NFT no sea el comprador. Incluir un mensaje de falla.
...
// Transferencia del precio de venta del comprador al propietario actual del NFT usando fundsToken.
        if ((!foundsToken.transferFrom(_msgSender(), ownerOf(tokenId), tokensale.price))){
...
// Transferencia de tarifa de comercio (calculada como un porcentaje del valor del NFT) del comprador al feesCollector.
       if ((!foundsToken.transferFrom(_msgSender(), feesCollector. tokenSale, tokensOnSale.price * tradeFee /10000))) {
            revert("no se pudo transferir la tarifa"); // Incluir un mensaje de falla.
        }
  
        emit Trade(_msgSender(), ownnerOf(tokenId), tokenId, tokenOnSale.price); // Registro de dirección del comprador, dirección del vendedor, tokenId, y precio de venta.  
        ...
        removeFromArray(listTokenOnSale, tokenId); // Remover el tokenId de la lista listTokensOnSale de NFTs.
...
// Función para poner en venta un NFT.
    function putOnSale(uint256 tokenId, uint256 price) external { // Parámetros: ID y precio del token.
        ...
        require(_msgSender()== ownerOf(tokenId), "El remitente no es propietario del token"); // Verificción remitente de la transacción es propietario del token. Incluir un mensaje de falla.
        ...
        addToArray(listTokenId, tokenId); // Añadir token a la lista.
...
// Función de retiro de venta de NFT.
    function removeFromSale(uint256 tokenId) external { // Parámetro: ID del token.
        ...
        require(_msgSender()== ownerOf(tokenId), "El remitente no es propietario del token"); // Verificación del _msgSender() es el propietario del token. Incluir un mensaje de falla.
        ...
        removeFromArray(listTokenOnSale, tokenId); // Eliminar el tokenId de la lista.
...
// Utilización del token ERC20 para transacciones.
    function setFundsToken(address _fundsToken) external onlyOwner { // Parámetro, token: Que va a ser la Dirección del contrato del token ERC20.                                                      
        require(_fundsToken != address(0), "Direccion invalida, no puede ser la direccion 0"); // La dirección no puede ser la dirección cero (address(0)). Incluir un mensaje de falla.
...
// Dirección para colectar los fondos de las ventas de NFTs.
    function setFundsCollector(address _fundsCollector) external onlyOwner { // Parámetro, dirección de colector de fondos.
        require(_fundsCollector != address(0), "Direccion invalida, no puede ser la direccion 0" ); // La dirección no puede ser la dirección cero (address(0))
...
// Dirección para colectar las tarifas de transacción.
    function setFeesCollector(address _feesCollector) external onlyOwner { // Parámetro, dirección del colector de tarifas.
        require(_feesCollector != addres(0), "La direccion no puede ser la 0"); // La dirección no puede ser la dirección cero (address(0))
        feesCollector = _add_feesCollector; // Dirección proporcionada a la variable feesCollector.
    }

// Porcentaje de beneficio a pagar en las reclamaciones.
    function setProfitToPay(uint256 _profitToPay) external onlyOwner { // Parámetro, porcentaje de beneficio a pagar.
...
// Función para eliminar un valor válido previamente agregado.
    function deleteValidValues(uint256 value) external onlyOwner { // Parámetro, valor que se quiere eliminar de los valores válidos.
...
// Tarifa aplicada a las compras de NFTs.
    function setBuyFee(uint256 _buyFee) external onlyOwner { // Parámetro, porcentaje de tarifa para compras.
...
// Retiro por parte del propietario de tokens ERC20.
    function recoverERC20(address erc20address) external onlyOwner { // Parámetro, dirección del contrato del token ERC20 que se desea recuperar.

// Transferir el balance completo (IERC20(token).balanceOf(address(this))) al propietario (owner()).
         if (!IERC20(erc20address).transfer(owner(), IERC20(erc20address).balanceOf(addres(this)))){ // Llamamos a la función transfer del contrato ERC20
             revert("Cannot send funds."); // Incluir un mensaje de falla.
         }
    }
...
// Buscar un valor en un array y retornar su índice o la longitud del array si no se encuentra.
    function find(uint256[] storage list, uint256 value) private pure returns(uint)  { // Parámetros, array de enteros en el cual se buscará el valor y valor que se buscará en el array..
